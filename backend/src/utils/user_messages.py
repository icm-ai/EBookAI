...
class UserMessageFormatter:
    ...
    ERROR_MESSAGES = {
        ...
        "FILE_NOT_FOUND": {
            ...
            "suggestions": ["确认文件路径拼写正确", "检查文件是否已被移动或删除", "尝试重新上传文件"],
        },
        "FILE_PROCESSING_ERROR": {
            "title": "文件处理失败",
            "message": "处理您的文件时出现问题，可能是文件格式不受支持或文件已损坏。",
            "suggestions": ["确认文件格式是否受支持（EPUB、PDF、TXT）", "检查文件是否完整且未损坏", "尝试使用其他文件"],
        },
        # Validation errors
        "VALIDATION_ERROR": {
            "title": "输入验证失败",
            "message": "您提供的信息格式不正确，请检查输入内容。",
            "suggestions": ["检查必填字段是否完整", "确认数据格式符合要求", "参考API文档中的示例"],
        },
        # Conversion errors
        "CONVERSION_ERROR": {
            "title": "格式转换失败",
            "message": "无法将文件转换为目标格式，请检查文件内容和格式要求。",
            "suggestions": ["确认源文件格式正确", "检查目标格式是否受支持", "如果是大文件，请稍后重试"],
        },
        "CONVERSION_TIMEOUT": {
            "title": "转换超时",
            "message": "文件转换时间过长，可能是文件太大或服务器繁忙。",
            "suggestions": ["尝试转换较小的文件", "稍后重试", "联系技术支持获取帮助"],
        },
        # AI service errors
        "AI_SERVICE_ERROR": {
            "title": "AI服务暂时不可用",
            "message": "AI处理服务遇到问题，请稍后再试。",
            "suggestions": ["等待几分钟后重试", "检查网络连接", "如问题持续存在，请联系支持团队"],
        },
        # Configuration errors
        "CONFIGURATION_ERROR": {
            "title": "系统配置错误",
            "message": "服务配置有误，请联系管理员。",
            "suggestions": ["联系系统管理员", "稍后重试", "查看系统状态页面"],
        },
        # Security errors
        "SECURITY_ERROR": {
            "title": "访问被拒绝",
            "message": "您没有权限执行此操作。",
            "suggestions": ["检查您的访问权限", "联系管理员获取权限", "确认您已正确登录"],
        },
        # Generic errors
        "INTERNAL_ERROR": {
            "title": "系统内部错误",
            "message": "系统遇到了意外错误，我们正在调查此问题。",
            "suggestions": ["稍后重试", "如问题持续存在，请联系技术支持", "记录错误ID以便技术支持定位问题"],
        },
        "HTTP_ERROR": {
            "title": "请求错误",
            "message": "您的请求格式不正确或包含无效数据。",
            "suggestions": ["检查请求参数", "参考API文档", "确认请求方法正确"],
        },
    }
    ...
    def format_error_response(
        ...
        message_template = cls.ERROR_MESSAGES.get(
            error_code, cls.ERROR_MESSAGES["INTERNAL_ERROR"]
        )
    ...
    def format_success_response(
        ...
        return {
            "success": True,
            "message": {"type": message_type.value, "text": message},
            "data": data,
        }
    ...
    def format_info_response(
        ...
        response = {"message": {"type": message_type.value, "text": message}}
...
def create_user_friendly_error(
    error: Exception, request_id: str = None, include_technical: bool = False
) -> Dict[str, Any]:
    """Convenience function to create user-friendly error response"""
    return UserMessageFormatter.format_error_response(
        error=error, request_id=request_id, include_technical_details=include_technical
    )


def create_success_message(data: Any, message: str = "操作成功完成") -> Dict[str, Any]:
    """Convenience function to create success response"""
    return UserMessageFormatter.format_success_response(data=data, message=message)
